...
# Usage:
# /applications/R/R-3.4.0/bin/Rscript features_EnrichedHeatmap_sorted_by_sRNAs.R CS+_2_LIB18613_LDI16228 2000 Copia_LTR_RLC 1400 2000 2kb '2 kb' 20 20bp 20 promoters

#libName <- "CS+_2_LIB18613_LDI16228"
#featureNumber <- 2000
#featureName <- "Copia_LTR_RLC"
#bodyLength <- 1400
#upstream <- 2000
#downstream <- 2000
#flankName <- "2kb"
...
#binSize <- 20
#binName <- "20bp"
#sRNAsize <- "20" # [20, 21, 22, 23, 24, 33, 34]
#region <- "promoters"
...
plotDir <- "./plots/"
sRNAsizePlotDir <- paste0(plotDir, sRNAsize, "nt_sorted/")
regionPlotDir <- paste0(sRNAsizePlotDir, region, "/")
famPlotDir <- paste0(regionPlotDir, featureName, "/")
system(paste0("[ -d ", plotDir, " ] || mkdir ", plotDir))
system(paste0("[ -d ", sRNAsizePlotDir, " ] || mkdir ", sRNAsizePlotDir))
system(paste0("[ -d ", regionPlotDir, " ] || mkdir ", regionPlotDir))
system(paste0("[ -d ", famPlotDir, " ] || mkdir ", famPlotDir))
plotDir <- famPlotDir

# Load matrix and extract region for sorting of features
mat1 <- as.matrix(read.table(paste0("../matrices/",
                                    libName,
                                    "_MappedOn_wheat_v1.0_both_",
                                    sRNAsize, "nt_sort_norm_",
...
sRNAmats <- mclapply(seq_along(sRNAsizes), function(x) {
  as.matrix(read.table(paste0("/home/ajt200/analysis/wheat/sRNAseq_meiocyte_Martin_Moore/snakemake_sRNAseq/mapped/TEProfiles/matrices/",
                              libName,
                              "_MappedOn_wheat_v1.0_both_",
...
ChIPDirs1 <- c(
               "/home/ajt200/analysis/wheat/H3K9me2/snakemake_ChIPseq/mapped/TEProfiles/matrices/",
               "/home/ajt200/analysis/wheat/H3K4me3/snakemake_ChIPseq/mapped/TEProfiles/matrices/",
               "/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/CENH3/snakemake_ChIPseq/mapped/TEProfiles/matrices/"
              )
ChIPDirs2 <- c(
               "/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/H3K4me3/snakemake_ChIPseq/mapped/TEProfiles/matrices/",
               "/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/H3K9ac/snakemake_ChIPseq/mapped/TEProfiles/matrices/",
               "/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/H3K27me3/snakemake_ChIPseq/mapped/TEProfiles/matrices/",
               "/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/H3K36me3/snakemake_ChIPseq/mapped/TEProfiles/matrices/"
              )
controlDirs <- c(
                 "/home/ajt200/analysis/wheat/MNase/snakemake_ChIPseq/mapped/TEProfiles/matrices/",
                 "/home/ajt200/analysis/wheat/epigenomics_shoot_leaf_IWGSC_2018_Science/input/snakemake_ChIPseq/mapped/TEProfiles/matrices/"             
                )
...
foreach(x = 1:5) %dopar% {
  sRNA_col_fun <- colorRamp2(quantile(c(sRNAmatsSorted[[x]]),
                                      c(0.99975, 0.9998, 0.999825, 0.99985, 0.999875, 0.9999),
                                      na.rm = T),
                             rich8to6equal)
  pdf(paste0(plotDir, sRNAsizes[x], "nt_sRNAs_around_top", featureNumber, featureName,
             "_heatmap_ordered_by_",
             libName, "_", sRNAsize, "nt_sRNAs_in_", region, ".pdf"),
      width = 5, height = 10)
  featureHeatmap(matSorted = sRNAmatsSorted[[x]][1:featureNumber,],
...
# 33-nt sRNAs
  sRNA_col_fun <- colorRamp2(quantile(c(sRNAmatsSorted[[6]]),
                                      c(0.99997, 0.9999735, 0.9999745, 0.9999755, 0.9999765, 0.9999775),
                                      na.rm = T),
                             rich8to6equal)
  pdf(paste0(plotDir, sRNAsizes[6], "nt_sRNAs_around_top", featureNumber, featureName,
             ...
             libName, "_", sRNAsize, "nt_sRNAs_in_", region, ".pdf"),
      width = 5, height = 10)
  featureHeatmap(matSorted = sRNAmatsSorted[[6]][1:featureNumber,],
                 col_fun = sRNA_col_fun,
...
# 34-nt sRNAs
  sRNA_col_fun <- colorRamp2(quantile(c(sRNAmatsSorted[[7]]),
                                      c(0.999977, 0.999978, 0.999979, 0.99998, 0.999981, 0.999982),
                                      na.rm = T),
                             rich8to6equal)
  pdf(paste0(plotDir, sRNAsizes[7], "nt_sRNAs_around_top", featureNumber, featureName,
             ...
             libName, "_", sRNAsize, "nt_sRNAs_in_", region, ".pdf"),
      width = 5, height = 10)
  featureHeatmap(matSorted = sRNAmatsSorted[[7]][1:featureNumber,],
                 col_fun = sRNA_col_fun,
...
foreach(x = 1:7) %dopar% {
  ...
  pdf(paste0(plotDir, ChIPNames[x], "_around_top", featureNumber, featureName,
             ...
             libName, "_", sRNAsize, "nt_sRNAs_in_", region, ".pdf"),
      width = 5, height = 10)
  featureHeatmap(matSorted = log2ChIPmatsSorted[[x]][1:featureNumber,],
...
#             libName, "_", sRNAsize, "nt_sRNAs_in_", region, ".pdf"),
#      width = 5, height = 10)
#  featureHeatmap(matSorted = log2ChIPmatsSorted[[3]][1:featureNumber,],
#                 col_fun = ChIP_col_fun,
...
# MNase
  ...
  pdf(paste0(plotDir, ChIPNames[8], "_around_top", featureNumber, featureName,
             ...
             libName, "_", sRNAsize, "nt_sRNAs_in_", region, ".pdf"),
      width = 5, height = 10)
  featureHeatmap(matSorted = log2ChIPmatsSorted[[8]][1:featureNumber,],
                 col_fun = ChIP_col_fun,
